         ...
         <div class="flex justify-start items-start">
          ...
          <form class="w-full space-y-6" @submit.prevent="register">
            <div class="mb-4">
              ...
              <input type="text" id="phone" v-model="credential.phone.value" class="w-full px-4 py-4  bg-[#F4F5F7] outline-none border-[0.5px] border-[#F4F5F7] rounded-md focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-4">
              <label class="block text-[#7D8799] font-medium mb-1 text-sm" for="bvn">Enter your BVN</label>
              <input type="phone" id="bvn" v-model="credential.bvn.value" class="w-full px-4 py-4  bg-[#F4F5F7] outline-none border-[0.5px] border-[#F4F5F7] rounded-md focus:outline-none focus:border-green-500" />
              <p class="text-[#8F9BB3] text-sm mt-1">Check your BVN via *565*0#</p>
            </div>
            <div class="pt-6">
              <button :disabled="loading || isFormDisabled" type="submit" class="w-full disabled:cursor-not-allowed disabled:opacity-25 bg-[#2F6D67] text-white py-3.5 rounded-md hover:bg-[#2F6D67] transition">{{loading ? 'processing...' : 'Create New Account'}}</button>
            </div>
          </form>
